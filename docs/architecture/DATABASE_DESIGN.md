...
| Milk | Implemented: milk_production_slots, milk_entries. Future: custom milk_sessions, milk_collection_batches, milk_batch_sources, milk_tank_movements, milk_quality_tests, milk_restrictions | Slot uniqueness prevents duplicate animal/date/session entries; immutable revisions preserve corrections; tank balance derives from future movements; restricted quantity is excluded from future sellable stock |
...
| Finance | Implemented: system_accounts, finance_journal_entries, finance_journal_lines, expense_records, income_records. Future: fiscal_periods, reversals, approvals, cost centres | PKR-only balanced double-entry journals; hidden Farm Funds avoids user-facing cash/bank setup; posted entries immutable and farm scoped |
| Workforce | Implemented: employees, payroll_periods, payroll_entries, employee_loans, employee_loan_installments. Future: attendance, leave, shifts, documents, tasks | Monthly salaries only; payroll requires approval before payment; payment recovers loans and posts one balanced journal |
