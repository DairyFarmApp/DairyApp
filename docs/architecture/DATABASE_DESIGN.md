...
| Finance | Future: system_accounts, journal_entries, journal_lines, expenses, income_records, fiscal_periods | PKR-only balanced double-entry journals; a hidden Farm Funds account avoids user-facing cash/bank setup; posted entries immutable; dimensions include farm/cost centre |
| Workforce | Future: employees, attendance, payroll_periods, payroll_entries, employee_loans, employee_loan_installments | Monthly salaries only; sensitive fields restricted; approved payroll and loan recovery post balanced finance entries |
