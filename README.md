...
Phase 2A provides the animal registry. Phase 2B adds controlled movements. Phase 2C adds exact decimal kg/lb weights, immutable corrections, latest-weight projection, dedicated versioned operational-status changes, responsive Flutter history/forms, and authorized Drift read caching. MySQL 8.4.9 passes all 67 backend tests with 677 assertions; Flutter passes 62 tests, analysis, and the Android debug build. See the [animal-registry guide](docs/ANIMAL_REGISTRY.md), [movement guide](docs/ANIMAL_MOVEMENTS.md), [weight/status guide](docs/ANIMAL_WEIGHTS_AND_STATUS.md), and [Phase 2C completion record](docs/PHASE_2C_COMPLETION.md).

## Phase controls

Read `AGENTS.md`, the master specification, and `docs/IMPLEMENTATION_PLAN.md` before changes. Phase 2C is implemented locally; do not begin Phase 2D or another animal/domain capability, commit, or push without explicit project-owner approval.
